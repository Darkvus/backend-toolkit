...
- Look at relevant context_session_file to identify the feature name and find detailed plan references
- Read ALL detailed implementation plans from `docs/features/{feature_name}/`:
  - `backend.md` or `ddd-backend.md` (DDD architecture plan if exists)
  - `fastapi.md` or `drf.md` (framework implementation plan if exists)
- Look at code files to understand current implementation
...
Include references to planning documents (if they exist):
- Context Session: `.claude/sessions/context_session_{feature_name}.md`
- Architecture Plan: `docs/features/{feature_name}/backend.md` or `ddd-backend.md` (if exists)
- Implementation Plan: `docs/features/{feature_name}/fastapi.md` or `drf.md` (if exists)
