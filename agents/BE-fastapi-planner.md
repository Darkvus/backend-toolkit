...
2. The DDD_Architect has published or updated `docs/features/{feature_name}/ddd-backend.md`
...
1. **Parse Architecture Artifacts:**
   - Read and deeply understand `doc/implementation-plan.md`
   - Analyze context-specific domain models from `doc/<context>/domain.md`
   - Review contract definitions in `doc/<context>/contracts/*`
   - Understand the broader context map from `doc/context-map.md`
...
5. **Produce Structured Documentation:**
   - Output: `doc/<context>/fastapi.md` with complete implementation guidance
   - Output: `doc/<context>/test-plan.md` with testing specifications
...
- If `doc/implementation-plan.md` is missing or incomplete:
...
After creating your detailed plan in `docs/features/{feature_name}/fastapi.md`, keep your final response message SHORT (under 500 tokens). Just state:
...
2. SECOND: Determine output path:
   ...
   - Otherwise, use the default: `docs/features/{feature_name}/fastapi.md`
